...
import (
	...
	"errors"
	"fmt"
	"github.com/FinnaCloud/finna-pk/commands"
...
func run() int {
	...
	//loginCmd.Flags().StringVar(&configPathArg, "config-path", "", "Path to the client config file. Default: ~/.finna-pk/config.yml on linux and %APPDATA%\\.finna-pk\\config.yml on windows")
	rootCmd.Flags().BoolVar(&createConfigArg, "create-config", false, "Creates a client config file if it does not exist")
	...
	//rootCmd.AddCommand(ro)

	//	readhomeCmd := &cobra.Command{
	//		SilenceUsage: true,
	//		Use:          "readhome <principal>",
	//		Short:        "Read the principal's home policy file",
	//		Long: `Read the principal's policy file (/home/<principal>/.finna-pk/auth_id).
	//
	//You should not call this command directly. It is called by the opkssh verify command as part of the AuthorizedKeysCommand process to read the user's policy  (principals) home file (~/.finna-pk/auth_id) with sudoer permissions. This allows us to use an unprivileged user as the AuthorizedKeysCommand user.
	//`,
	//		Args:    cobra.ExactArgs(1),
	...
	//		Long: `Verify extracts a PK token from a base64-encoded SSH certificate and verifies it against policy. It expects an allowed provider file at /etc/finna-pk/providers and a user policy file at either /etc/finna-pk/auth_id or ~/.finna-pk/auth_id.
	//
	...
	//			typArg := args[2]
	//
	//			providerPolicyPath := "/etc/finna-pk/providers"
	//			providerPolicy, err := policy.NewProviderFileLoader().LoadProviderPolicy(providerPolicyPath)
	//			if err != nil {
	//				log.Println("Failed to open /etc/finna-pk/providers:", err)
	//				return err
	//			}
	//
	//			printConfigProblems()
	...
	//		},
	//	}
	//	verifyCmd.Flags().StringVar(&serverConfigPathArg, "config-path", "/etc/finna-pk/config.yml", "Path to the server config file. Default: /etc/finna-pk/config.yml.")
	...
	//}
	//
	//providerListCmd.Flags().StringVar(&configPathArg, "config-path", "", "Path to the client config file. Default: ~/.finna-pk/config.yml on linux and %APPDATA%\\.finna-pk\\config.yml on windows.")
	//
