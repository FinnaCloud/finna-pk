...
For instance Docker uses the custom claim `att` in the CIC protected header to ensure [a particular PK Token can only be used to verify a particular object.](https://github.com/FinnaCloud/finna-pk/issues/33)
...
For the complete details on the GQ signing see our package [gq.SignJWT](https://github.com/FinnaCloud/finna-pk/blob/main/gq/sign.go#L108)
...
Similar to our approach of GQ signatures, we can use ZKP (Zero Knowledge Proofs) to provide privacy-enhanced PK Tokens, and if needed use the ZKP as a binding mechanism. This is currently under discussion and development in the issue: [Proposed zklogin JWS](https://github.com/FinnaCloud/finna-pk/issues/101).
